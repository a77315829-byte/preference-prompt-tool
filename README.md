...
사용자의 암묵적 선호를 추정해 프롬프트를 조립한다. 추정 선호를 평가 함수로
변환해 최적화기([GEPA](https://arxiv.org/abs/2507.19457))에 전달하는 단계는 선택 사항이다. 최종
...
![첫 화면](docs/screenshots/00-hero.png)
...
마지막으로 이 결과가 취향에 맞았는지 한 줄로 묻는다. 지금까지의 정량
결과는 전부 MACSum 페르소나 기반이고 "실제 사람도 좋아했다"는 근거가
없었다. 공개 링크 방문자의 응답이 그 표본이 된다. 피드백 로그에는 원문과 생성 결과를
넣지 않고, 예/아니오·도메인·실행 모드·추정된 선호·선택 의견만
로그에 남긴다 (`feedback.py`). 별도로 API 생성 결과는 서버 파일 캐시에 저장된다.
자유 의견에는 개인 정보를 적지 않도록 주의해야 하며, 로그 집계만으로 익명성을 보장하지 않는다. 집계는
...
#### 1. GEPA 피드백 풍부도 어블레이션 — 최적화 지표에서 관찰한 차이
...
유지하는 반면, terse는 해당 실행에서 정체된다. 이 점수는 **최적화에 사용한
평가 함수의 점수**다. 소규모 단일 실험에서 관찰한 차이이며 독립적인 품질
향상이나 실사용자 만족도, 일반적인 인과효과를 입증하지 않는다. 반복 시드와
분리된 평가 자료를 사용한 검증이 남아 있다.
...
| B - 실험자가 작성한 고정 일반 지침 | 0.668 ± 0.240 | 0.168 ± 0.060 |
...
**B 의 지침 원문** — 실험자가 작성한 일반 지침 하나를 5개 문서에 똑같이
사용했다. 실제 사용자가 자기 취향을 알고 작성한 지침과의 비교는 아니다.
이 문구를 공개하는 것만으로 비교군의 강도나 공정성이 입증되지는 않는다.

```
Summarize the following article in a clear and concise way that captures the key points.
```

**D 는 GEPA 를 쓰지 않는다.** 8회 선택 루프 → 추정된 선호로 시드 프롬프트
조립 → 생성. 위 수치는 템플릿 조립본의 결과이고, GEPA 의 효과는 1번
어블레이션에서만 잰다. GEPA 가 대표 수치에 기여했다고 읽으면 틀린다.

#### 2-1. 검사 함수는 사람 라벨을 재현하지 못한다 — 순환 논증 방어를 위해 잰 것

D 는 `checks/` 로 프롬프트를 만들고 같은 `checks/` 로 채점한다. 검사 함수가
사람 라벨과 어느 정도 맞는지 없이는 그 의심을 끊을 수 없어서 쟀다
(`python -m experiments.checks_vs_labels`, macdoc 사람 요약 5,379개, API 0회).

| 축 | 값 | 라벨 검사 통과율 | 원시값 평균 | 균형정확도 |
|---|---|---|---|---|
| length | short / normal / long | 81% / **20%** / 32% | 1.74 / 2.23 / 3.96 문장 | **0.446** |
| extractiveness | normal / high / fully | 67% / 39% / 78% | 0.44 / 0.55 / 0.84 | **0.598** |
| (specificity) | normal / high | — | 0.217 / 0.228 | 0.500 (분류기) |

기존 체크는 사람 라벨과 제한적으로 일치한다. 이 분석은 train/val/test를
합쳐 수행했고 이전 임계값 보정에 사용한 자료도 포함하므로 **독립 홀드아웃
검증이 아니다**. 탐색한 단일 측정값 임계값 중 최고 성능 0.49 / 0.61도
그 탐색 범위의 결과이며, 다른 접근의 성능 상한이나 측정 불가능의 증거가 아니다.
라벨 평균의 차이만으로 판별력이나 그 원인을 확정할 수 없다.

따라서 정확히 적으면: 페르소나 실험이 검증한 것은 "MACSum 사람의 선호
복원"이 아니라 **"YAML 이 정의한 값 아래에서 선택·추정 기계가 숨긴 선호를
복원하는가"** 다. 오라클과 후보 생성 모두 같은 정의를 쓰므로 그 루프는
자기 일관성을 측정한다. 외부 라벨과 대조해도 **같은 검사 함수를 사용하는
자기평가의 한계는 사라지지 않는다**. ROUGE-L은 참고 요약과의 텍스트 겹침을
보완하지만 사실성·사용자 만족도의 직접 측정은 아니다.
`tests/test_checks_vs_labels.py`는 수치 계산의 회귀를 확인한다.
...
#### 5. 전문가 프롬프트: 자기 글을 넣으면 그 형식으로 쓰는 프롬프트

비교 루프와 출발점이 다른 두 번째 경로다. 사용자가 **이미 써둔 답변**을
붙여넣으면 형식을 코드로 재서, 그 형식으로 쓰게 하는 프롬프트를 만든다
(`agents/expert_onboarding.py`, `expert_profile.py`).

**검증은 실제 사람으로 했다.** Stack Exchange 공개 API 로 네 분야(요리·
글쓰기·수리·학계)의 상위 답변자 16명을 받았다 (`experiments/expert_corpus_se.py`,
키 없음·새 의존성 0개). 한 사람의 답변 30개를 보여주고 프롬프트를 만든
뒤, **한 번도 보여주지 않은 질문 30개**에 적용해 그 사람의 실제 답변과
형식을 비교한다.

| 조건 | 형식 거리 | 개선된 저자 | 부호검정 p | 프롬프트 |
|---|---|---|---|---|
| 지시 없음 | 0.308 | - | - | 16단어 |
| **측정한 앵커** | **0.146** | **8/8** | **0.008** | **29단어** |
| 저자 답변 3개를 예시로 | 0.222 | 7/8 | 0.070 | 1,010단어 |
| 예시 + 앵커 | 0.149 | 8/8 | 0.008 | 1,022단어 |

**형식은 전이되고 내용은 전이되지 않는다.** ROUGE-L 은 0.135 에서 0.145~
0.146 으로 거의 움직이지 않는다 - Q&A 에서 답변 내용은 질문이 정하므로
형식을 맞춰도 같은 문장이 나오지 않는다. 그래서 이 기능이 맞추는 것은
형식이라고 명시한다.

**예시를 보여주는 것보다 수치를 재는 것이 낫다.** "그냥 자기 글 몇 개를
예시로 넣으면 되지 않나"는 가장 먼저 나올 질문인데, 예시만으로는 앵커를
이기지 못한다(0.222 대 0.146). 프롬프트를 35배 써도 그렇다. 예시는 문체를
보여주지만 "이 과제에 몇 단어를 쓸지"를 지정하지 못한다.

둘을 합쳐도 나아지지 않는다(0.149). 학습 자료가 12개뿐일 때는 예시가
24% 기여했는데, 30개로 늘려 앵커의 목표가 정확해지면 기여가 사라진다 -
**예시와 수치는 보완이 아니라 대체 관계**이고, 자료가 적을 때만 예시가
값을 한다.

**그래서 배포된 앱은 앵커만 쓰고 모델을 호출하지 않는다.** 측정이 전부
파이썬이라 모델 호출이 0회다. **입력은 Streamlit 서버로 전송되어 처리되며,
이 기능에서 모델 API로 보내지 않는다.**
프롬프트에 들어가는 것은 측정한 수치 29단어뿐이다. 개인 자료를 다루는
기능에서 이건 부수 효과가 아니라 설계상의 이점이다
(`tests/test_expert_flow_app.py` 가 이 경로에서 API 함수를 호출하면 실패한다).

**도메인 의존성을 하나 찾아냈다.** 초기 버전은 MACSum 요약으로 검증해
"원문 대비 압축률로 재라"는 결론을 냈고 n=30 부호검정까지 붙였는데,
실제 Q&A 에서는 방향이 반대였다.

| 코퍼스 | 상관(과제 길이, 답변 길이) | 압축률 판별력 |
|---|---|---|
| MACSum (요약) | 0.91 ~ 0.996 | **165.5** |
| 실제 Q&A 4개 분야 | 0.04 ~ 0.25 | 0.38 ~ 1.72 (16명 전원 기각) |

요약문은 원문에서 파생되므로 길이가 원문을 따라가지만, 답변 길이는
**질문의 난이도**가 정하고 질문의 길이가 정하지 않는다. 그래서 어느
모수화를 쓸지도 상수로 박지 않고 학습 예시에서 재서 고른다
(`length_parameterization` - 저자 11명 x 20개 분할 = 220 케이스에서
전부 정답).

**필요 자료는 30개다.** 12개로는 한 저자의 평균 길이를 465단어로 잡았는데
실제는 331단어였고, 그 저자에서 앵커가 아무것도 안 한 것보다 나빴다.
홀드아웃을 고정하고 학습만 30개로 늘리니 그 저자가 −0.122 에서 +0.187 로
뒤집혔다.
...
#### 6. specificity 축: 다섯 가지 방법 전부 실패
...
#### 7. 도메인 온보딩 에이전트 (연구용 MVP): 실행 루프와 사람 축의 일치 여부

**2026-09-18 재점검:** 기존 6개 사례 통과가 안전성 보장은 아니었다.
`statistics.sys` 같은 모듈 속성 접근을 추가로 발견해 허용 멤버만 노출하고
비공개 속성·문자열 포맷을 통한 속성 탐색을 제한했다. 자식 프로세스에 앱의
API 키 환경변수를 전달하지 않고 NaN/무한대 측정값도 거부한다.
여전히 OS 수준 파일·네트워크·메모리 격리가 없으므로 **공개 비신뢰 코드 실행용으로
사용하지 않는다**. 기존 방어 실험은 아래에 이력으로 남긴다.
...
실행 피드백에 따라 도구 사용을 반복하는 에이전트형 루프다. 재시도 상한은 코드에 정의되어 있다.
...
**결론**: 라벨 없이 측정값이 변하는 축을 제안·실행·선별하는 루프는 작동한다.
값의 분산만으로 사용자에게 유용한 축임이 입증되지는 않는다 - 이건 사람이 만든 것과 독립적인
...
agents/              도메인 온보딩 에이전트 · 전문가 프롬프트 (engine/과 분리)
tests/               각 부품·엔드투엔드 검증 스크립트
scripts/             일회성 탐색 스크립트 (예: MACSum 데이터 훑어보기)

  --- UI 를 모르는 층 (Streamlit 을 import 하지 않는다) ---
service.py           비교 루프. start_session / submit_choice / optimize
expert_profile.py    붙여넣은 글 → 형식 프로필 → 앵커 프롬프트
budget.py            호출 상한 (세션·하루)
feedback.py          실사용자 응답 기록

  --- UI 층 ---
theme.py             CSS 생성 · 디자인 토큰 · 버전 의존 선택자 목록
app.py               Streamlit UI. 위 층들을 호출만 한다
```
...
**UI 를 모르는 층을 따로 둔다.** `service.py` 는 Streamlit 을 import 하지
않고, `SessionState` 는 엔진 객체를 담지 않는 순수 dataclass 다 - 선택
이력만 들고 있다가 필요할 때 재생해서 복원한다. 그래서 그대로 직렬화되고,
나중에 FastAPI 세션 저장소로 옮길 때 이 층은 손댈 필요가 없다. 재생이
같은 결과를 내는 이유는 `UncertaintySelector` 의 RNG 가 호출마다 진행되기
때문이고, `tests/test_service.py` 가 그 재현성을 고정한다.
...
라운드 스테퍼와 축별 게이지가 붙는다.

**게이지는 확신도가 아니라 "그 축이 갈린 비교 횟수"를 보여준다.**
확신도(`estimator.confidence()`)를 쓰지 않는 이유는 실측이다 - 3값 축은
비교를 24회까지 늘려도 0.06 에 머무는데 선호는 9/9 정확히 복원한다
(아래 「보너스: 사용자에게 보여줄 확신도」). 차오르는 바에 그 값을 걸면
6% 에서 6% 로 움직이고, 맞힌 사용자에게 "확신도 6%"라고 말하게 된다.
갈린 비교 횟수는 코드로 셀 수 있고, 단조 증가하고, 축마다 다르게
차오르며, 확률 주장을 하지 않는다 - 엔진이 이미 쓰는 개념과도 같다
(`Estimator.update` 는 두 후보의 축값이 같으면 그 축을 건너뛴다).

게이지 개수는 도메인이 정한다. 자유 키워드 축(요약의 topic)은 추정값·
확신도 모델에 맞지 않아 빠지므로 요약은 2개, 코딩은 3개다.

**카드 높이는 고정이다.** 후보 길이에 따라 레이아웃이 흔들리면 사용자가
내용이 아니라 흔들림을 보고 고르게 되고, 그게 곧 length 축 판단 오염이다. 무료 데모 모드는 API 키나 비용 없이 전체 흐름을
실행하고(`demos/`의 규칙 기반 생성기), OpenAI API 모드는 실제로 모델을
호출해 후보를 만든다. **다섯 카테고리 모두** 두 모드를 지원한다.

**두 번째 경로: 내가 쓴 글에서 만들기.** 비교할 자료가 없어도 되는 위
...
1.90 → 0.88) 길이·문장이 나빠진다(형식 거리 0.158 → 0.256). 그 상충을
접기에 수치로 적어 사용자가 고를 수 있게 뒀고, 기본값은 저자 8명에게
검증된 조건(길이·문장만)이다.

측정 결과와 실험 수치는 위 「5. 전문가 프롬프트」에 있다.
...
2026-09-18 코드 점검, 남은 위험, 카테고리별 검증 계획, 사용자에게 필요한 자료는
[프로젝트 점검 및 보완 계획](docs/project_review_2026-09-18.md)에 정리했다.
GEPA 시도는 브라우저 세션당 2회·앱 프로세스 하루 30회로 제한하며 실패도 차감한다.
이는 실행 횟수 제한이며 토큰·금액 상한이 아니고 앱 재시작 시 하루 카운터가 초기화된다.


## 한국어 요약 실사용자 검증 준비

[한국어 평가 실행 안내](docs/korean_pilot/README.md)에 따라 직접 작성한 지침과
8회 선택으로 조립한 지침을 **새 원문**에서 블라인드 비교할 수 있다.
이것은 검증을 수집하는 도구이며, 아직 한국어 개인화 효과를 입증한 결과는 아니다.

```powershell
# API 없이 연습 자료 생성
.\.venv\Scripts\python.exe -m experiments.korean_pilot demo
# 독립 평가 화면 (공개 app.py와 분리)
.\.venv\Scripts\python.exe -m streamlit run pilot_review.py --server.address 127.0.0.1
```

화면에서 `data/korean_pilot_demo/P001/blind_packet.json`을 업로드하면 된다.
실제 실험은 [입력 예시](docs/korean_pilot/protocol.example.json)를 실제 참여자
자료로 바꾸고 명시적으로 `--generate`를 사용한다. 원문 재사용·미응답·중복
참여자 집계를 검사하며, 연습 자료는 실사용자 집계에서 제외한다.
선호와 사실 오류·핵심 누락은 별도로 기록하고, 참여자 단위로 집계한다.
