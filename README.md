...
얼마나 일치하는지, 코드 기반 검사 함수로 0~1점 채점 (5개 문서 평균).

| 조건 | 평균 점수 | 표준편차 |
|---|---|---|
| A - 프롬프트 없이 기본 호출 | 0.600 | 0.224 |
| B - 사용자가 직접 쓴 일반적인 지침 | 0.668 | 0.240 |
| **D - 본 도구 (8회 비교 → 자동 조립된 프롬프트)** | **0.996** | **0.010** |

![비교군별 결과](experiments/results/baseline_comparison.png)

D는 사용자가 직접 지침을 쓴 B보다도 확연히 높고 편차도 훨씬 작다 - 선택
8회(클릭만 하면 됨, 지침을 언어로 표현할 필요 없음)로 직접 서술한 지침보다
더 안정적으로 개인 선호에 맞는 결과를 낸다.
