...
현재 MVP는 이 방식을 TypeScript/React 코딩 스타일과 AWS 비용 점검 개인화로 확장했다.
사용자가 코드 A/B 예시 중 더 마음에 드는 결과를 고르면 코드 구성, 스타일
관리, 타입 작성 선호를 반영한 재사용 가능한 시스템 프롬프트를 만든다. 문서 요약,
고객 리뷰, 이메일 초안도 같은 공통 엔진으로 선택할 수 있다. React 화면에는
문서 요약(영어/한국어), 코딩 도움, AWS 비용 점검, 고객 리뷰, 이메일 초안의
6개 카테고리가 카드로 노출된다.
...
콘솔에서 실행할 해결 방법을 함께 보여준다.

마지막으로 이 결과가 취향에 맞았는지 한 줄로 묻는다. 지금까지의 정량
결과는 전부 MACSum 페르소나 기반이고 "실제 사람도 좋아했다"는 근거가
없었다. 공개 링크 방문자의 응답이 그 표본이 된다. 원문과 생성 결과는
저장하지 않고, 예/아니오·도메인·실행 모드·추정된 선호·선택 의견만
익명으로 로그에 남긴다 (`feedback.py`). 집계는
...
**B 의 지침 원문** — 페르소나를 모르는 상태로 쓴 일반 지침 하나를 5개
문서에 똑같이 썼다. "일부러 약하게 쓴 것 아니냐"는 의심에 대한 답이다.
...
측정값 평균은 라벨 순서와 일치하지만(specificity 는 순서조차 없다), 라벨을
재현하지는 못한다. 임계값을 사람 분포에 맞춰 다시 골라도 0.49 / 0.61 이
상한인데, MACSum "long" 요약의 10분위가 1문장이라 **길이 라벨이 절대
문장 수가 아니기 때문**이다.
...
자기 일관적이고, ROUGE-L 이 독립 지표로 남는다. D 가 B 를 ROUGE-L 에서
이긴 것은 D 의 길이 정의가 사람 관례와 어긋나는 불리한 조건에서 나온
결과다. `tests/test_checks_vs_labels.py` 가 이 수치를 고정한다.
...
파이썬이라 호출이 0회이고, **사용자의 글이 어디로도 전송되지 않는다** -
...
#### 7. 도메인 온보딩 에이전트 (MVP): 안전한 발견은 성공, 사람과는 다른 축을 찾음
...
위 표는 기존 Streamlit 화면의 다섯 카테고리다. 새 React 화면에서는 여기에
AWS 비용 점검(`idle_tracker`)을 추가해 6개 카테고리를 제공한다. 연구용
`summarization_hybrid`와 `macsum_eval_agent` 도메인은 API/실험 전용이라
사용자 선택 카드에는 넣지 않았다.
...
형식 측정이 전부 파이썬이라 **모델을 호출하지 않고**, 따라서 붙여넣은 글은
어디로도 전송되지 않는다 - 프롬프트에 들어가는 것은 측정한 수치뿐이다
(`tests/test_expert_flow_app.py` 가 이 경로에서 API 함수를 호출하면 실패한다).
