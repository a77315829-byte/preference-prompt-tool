...
표면형으로는 다른 토큰이 된다. 여기서는 Kiwi로 형태소를 뽑아 내용어
(명사·동사·형용사·부사·어근)만 남기고 조사·어미·기호를 버린 뒤 비교한다.
"""
...
def lexical_overlap(output: str, source: str, value: str, target: dict) -> tuple[float, str]:
    output_bigrams = _bigrams(_content_morphemes(output))
    source_bigrams = _bigrams(_content_morphemes(source))
