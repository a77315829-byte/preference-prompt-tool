...
부수 효과가 아니라 설계상의 이점이다 - **사용자의 글이 어디로도 전송되지
않는다.** 개인 자료를 다루는 기능에서 이게 가장 중요하다.
...
압축률이 뽑히는 사용자(요약 위주 자료를 넣은 경우)는 과제마다 값이
달라지므로 그 사실을 화면에 알린다. 조용히 한 과제의 값을 박아 주면
다른 과제에서 틀린다.
"""
...
def readiness(count: int) -> tuple[str, str]:
    ...
    if count < RECOMMENDED_ANSWERS:
        return "thin", (
            f"{count}개로 계산했습니다. {RECOMMENDED_ANSWERS}개쯤 넣으면 더 정확합니다 - "
